...
const resolveLiveClassAudience = ({ body, instituteId }) => {
  const normalizedAudience = normalizeAudienceInput(
    ...
    },
    {
      defaultScope:
        body.batchId ||
        (Array.isArray(body.batchIds) && body.batchIds.length > 0)
          ? AUDIENCE_SCOPES.BATCH
          : body.instituteId || instituteId
            ? AUDIENCE_SCOPES.INSTITUTE
  ...
  try {
    ...
    if (hasOverlap) {
      return res.status(400).json({
        success: false,
        message:
          "Scheduling Conflict: You already have another live class scheduled that overlaps with this time slot.",
      });
    }
    ...
    if (
      ...
      return res.status(403).json({
        success: false,
        message:
          "Institute policy blocks global publishing for institute tutors",
      });
    }
    ...
    const liveClass = await LiveClass.create({
      ...
      courseId,
      batchId:
        audience.scope === AUDIENCE_SCOPES.BATCH
      ...
      instituteId:
        audience.scope === AUDIENCE_SCOPES.GLOBAL
          ...
          : audience.instituteId ||
            ...
            null,
      visibility:
        audience.scope === AUDIENCE_SCOPES.GLOBAL ? "public" : "institute",
      audience,
      dateTime,
      duration,
...
export const getLiveClasses = async (req, res) => {
  try {
    ...
    // If tutor, show ONLY their classes strictly
    if (req.user.role === "tutor") {
      // 🚨 FIX: Force strict lookup
      ...
      // Validate if specific course is requested, tutor MUST own it
      ...
      if (req.query.courseId) {
        const enrollment = enrollments.find(
          (e) => e.courseId.toString() === req.query.courseId,
        );
    ...
    if (
      (featureFlags.audienceEnforceV2 || featureFlags.audienceReadV2Shadow) &&
  ...
  try {
    ...
    if (dateTime || duration) {
      const newDateTime = dateTime
        ? new Date(dateTime)
        : new Date(liveClass.dateTime);
      const newDuration = duration || liveClass.duration;

      // Past Time check
      if (
        newDateTime < new Date() &&
        String(newDateTime) !== String(liveClass.dateTime)
      ) {
        return res.status(400).json({
          success: false,
      ...
      if (hasOverlap) {
        return res.status(400).json({
          success: false,
          message:
            "Scheduling Conflict: This new time overlaps with another of your scheduled classes.",
        });
      }
    }
  ...
  try {
    ...
    if (req.user.role === "tutor") {
      ...
      isTutorOwner = Boolean(
        tutor && liveClass.tutorId.toString() === tutor._id.toString(),
      );
      if (!isTutorOwner) {
        return res
          .status(403)
          .json({
            success: false,
            message: "Not authorized to join this class as tutor",
          });
      }
    }
    ...
    if (req.user.role === "student") {
      ...
      if (!enrollment && liveClass.courseId) {
        return res
          ...
          .json({
            success: false,
            message: "Enroll in the linked course to join this class",
          });
      }
      if (
        liveClass.batchId &&
        enrollment?.batchId?.toString() !== liveClass.batchId.toString()
      ) {
        return res
          .status(403)
          .json({
            success: false,
            message: "This class is not available for your batch",
          });
      }
      ...
      if (featureFlags.audienceEnforceV2) {
        ...
        if (!accessDecision.allowed) {
          return res.status(403).json({
            success: false,
            message:
              "This class is not available in your current audience scope",
          });
        }
      }
    }
    ...
    if (
      req.user.role === "admin" &&
      liveClass.instituteId &&
      req.tenant?._id?.toString() !== liveClass.instituteId.toString()
    ) {
      return res
        .status(403)
        .json({
  ...
  try {
    ...
    if (!enrollment && liveClass.courseId) {
      return res
        ...
        .json({
          success: false,
          message: "Enroll in the linked course to mark attendance",
        });
    }
    ...
    if (existing) {
      return res
        .status(200)
        .json({ success: true, message: "Attendance already marked" });
    }
