...
export const getClassAttendanceReport = async (req, res) => {
  try {
    ...
    const enrollmentQuery = { courseId: liveClass.courseId };

    // Updated to support new Audience Engine & Legacy fallback
    if (
      liveClass.audience?.scope === "batch" &&
      liveClass.audience.batchIds?.length > 0
    ) {
      enrollmentQuery.batchId = { $in: liveClass.audience.batchIds };
    } else if (
      liveClass.audience?.scope === "private" &&
      liveClass.audience.studentIds?.length > 0
    ) {
      enrollmentQuery.studentId = { $in: liveClass.audience.studentIds };
    } else if (liveClass.batchId) {
      // Legacy support (agar purani classes DB mein hain)
      enrollmentQuery.batchId = liveClass.batchId;
    }
...
export const markAttendance = async (req, res) => {
  try {
    ...
    if (
      batch.instituteId &&
      req.tenant?._id?.toString() !== batch.instituteId.toString()
    ) {
      return res
        .status(403)
        .json({
          success: false,
          message: "Not authorized to mark attendance for this institute",
        });
    }
    ...
    if (!isOwner) {
      return res
        ...
        .json({
          success: false,
          message: "Not authorized to mark attendance for this batch",
        });
    }
    ...
    let attendance = await BatchAttendance.findOne({
      batchId,
      date: targetDate,
    });
...
export const getBatchAttendance = async (req, res) => {
  try {
    ...
    if (
      ...
      !batch.students.some((studentId) => studentId.toString() === req.user.id)
    ) {
      return res
        .status(403)
        .json({ success: false, message: "You are not in this batch" });
    }
    ...
    if (req.user.role === "student") {
      processedRecords = attendanceRecords.map((doc) => {
        const studentRecord = doc.records.find(
          (r) => r.studentId._id.toString() === req.user.id,
        );
        return {
          _id: doc._id,
          date: doc.date,
